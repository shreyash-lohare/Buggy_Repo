...
async def get_analytics():
    ...
    if not items and not users: # ERROR CHECK FOR PLOTTING nothing to plot
        return JSONResponse(
            {
                "stats": {
                    "item_count": 0,
                    "user_count": 0,
                    "avg_item_name_length": 0.0,
                    "avg_user_username_length": 0.0,
                    "max_item_name_length": 0,
                    "max_user_username_length": 0,
                },
                "plot": None,
            }
        )
    ...
    item_name_lengths = (
        np.array([len(item.get("name", "")) for item in items])
        if items
        else np.array([])
    )
    user_username_lengths = (
        np.array([len(user.get("username", "")) for user in users])
        if users
        else np.array([])
    )
    ...
    return JSONResponse(
        {"stats": stats, "plot": f"data:image/png;base64,{image_base64}"}
    )
